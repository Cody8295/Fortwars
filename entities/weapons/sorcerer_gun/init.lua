...
function SWEP:PrimaryAttack()
  ...
  if self.Owner:GetNWInt('energy') > self.ManaCost then
    ...
    util.Decal("FadingScorch", pos + norm, pos - norm )
      ...
      PlyTakeEnergy(self.Owner, self.ManaCost)
  end
end
...
function SWEP:SecondaryAttack()
  if !PlyHasSpecial(self.Owner,11) then self.Owner:ChatPrint("You do not have the special ability for this class. Press F1 and look in the Classes tab to buy it.") return end
  if self.seeking or self.Owner:Energy() < SORCERER_MANA_COST then return end
  self.seeking = true
  self.lastSeek = CurTime() - 2
  PlyTakeEnergy(self.Owner, SORCERER_MANA_COST)
end